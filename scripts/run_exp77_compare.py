...
def main():
 out=Path("audit_outputs/exp77"); out.mkdir(parents=True,exist_ok=True); results={}
 for start,end in WINDOWS:
  key=f"{start}_{end}"; payload=retrain_and_compare(start,end,"exp_77"); overall=payload.get("overall_comparison") or {}; missing=[k for k in REQUIRED if overall.get(k) is None]
 ...
 (out/"summary.json").write_text(json.dumps({"experiment_id":"exp_77","windows":results,"promotion_allowed":False},indent=2,allow_nan=False)+"\n")
