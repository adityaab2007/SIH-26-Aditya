...
def main():
    out=Path("audit_outputs/exp75"); out.mkdir(parents=True,exist_ok=True)
    results={}
    for start,end in WINDOWS:
        key=f"{start}_{end}"
        payload=retrain_and_compare(start,end,"exp_75")
    ...
    (out/"summary.json").write_text(json.dumps({"experiment_id":"exp_75","windows":results,"promotion_allowed":False},indent=2,allow_nan=False)+"\n")
