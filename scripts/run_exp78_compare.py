"""Run Experiment 78 against fresh production baselines for required windows."""
import json
import math
import os
from pathlib import Path

from backend.app.ml.experiments import adapter_exp78
...
def main():
    out = Path("audit_outputs/exp78")
    out.mkdir(parents=True, exist_ok=True)
    results = {}
    for start, end in _selected_windows():
        key = f"{start}_{end}"
        payload = run_batch_comparison(adapter_exp78, start, end)
    ...
    summary = _safe({"experiment_id": "exp_78", "windows": results, "promotion_allowed": False})
