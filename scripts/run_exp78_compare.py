...
def main():
    ...
    for start, end in WINDOWS:
        key = f"{start}_{end}"
        payload = retrain_and_compare(start, end, "exp_78")
    ...
    (out / "summary.json").write_text(json.dumps(_safe({"experiment_id": "exp_78", "windows": results, "promotion_allowed": False}), indent=2, allow_nan=False) + "\n")
