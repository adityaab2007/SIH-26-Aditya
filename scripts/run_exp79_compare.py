...
def main():
    ...
    for start, end in WINDOWS:
        key = f"{start}_{end}"
        payload = retrain_and_compare(start, end, "exp_79")
    ...
    (out / "summary.json").write_text(json.dumps(_safe({"experiment_id": "exp_79", "windows": results, "promotion_allowed": False}), indent=2, allow_nan=False) + "\n")
