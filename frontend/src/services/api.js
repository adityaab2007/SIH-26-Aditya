...
async function request(path, options = {}, timeoutMs = DEFAULT_TIMEOUT_MS) {
  ...
  try {
    ...
    if (error?.name === 'AbortError') {
      throw new Error(timeoutMs === COMPARE_TIMEOUT_MS
        ? 'Retrain & Compare timed out. The pending comparison was discarded; retry the requested range.'
        : 'Request timed out. The backend may be busy; retry in a moment.');
    }
    throw error;
  } finally {
    clearTimeout(timer);
...
export const api = {
  ...
  improvedLifecycleSpecialistsRetrain: (startYear, endYear) => request('/api/models/experiments/lifecycle-specialists-improved/retrain', { method: 'POST', headers: JSON_HEADERS, body: JSON.stringify({ start_year: Number(startYear), end_year: Number(endYear) }) }, HEAVY_TIMEOUT_MS),
  ...
  improvedLifecycleSpecialistsComparison: (window = '2001_2015') => request(`/api/models/lifecycle-specialists-improved/${encodeURIComponent(window)}/comparison`),
  ...
  retrainAndCompare: (startYear, endYear, experimentId = null) => request('/api/model-simulations/custom/retrain-compare', {
    ...
    body: JSON.stringify({ start_year: Number(startYear), end_year: Number(endYear), experiment_id: experimentId }),
  }, COMPARE_TIMEOUT_MS),
