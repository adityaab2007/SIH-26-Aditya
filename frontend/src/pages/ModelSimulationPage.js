...
export async function ModelSimulationPage(root) {
  ...
  trainButton.addEventListener('click', async () => {
    ...
    try {
      ...
      receipt.innerHTML = `<strong>Model ${escape(registryRun.model_version)} trained and registered.</strong> Training: ${escape(registryRun.training_years)} · testing: ${escape(registryRun.testing_years)}. <strong>Fresh metrics:</strong> cost MAE ${fixed(registryRun.metrics.cost_model.MAE)} pp · delay MAE ${fixed(registryRun.metrics.delay_model.MAE_days)} days · risk F1 ${fixed(registryRun.metrics.risk_model.f1 * 100, 1)}%. <a href="#/prediction-accuracy">Open dynamic Prediction Accuracy</a><br><strong>Leakage guard:</strong> ${escape(session.leakage_guard)} Browser received actual held-out outcomes: <strong>${session.actual_outcomes_sent_to_browser ? 'YES' : 'NO'}</strong>.`;
