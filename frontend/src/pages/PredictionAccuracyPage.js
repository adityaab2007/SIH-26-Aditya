...
const formatRatioPercent = (number, digits = 1) => {
  if (number === null || number === undefined || number === '' || Number.isNaN(Number(number))) return 'N/A';
...
function runOptions(runs, selected) {
  return runs.map((run) => {
    const label = `${run.training_start}–${run.training_end}${run.complete ? '' : ` · ${String(run.status || 'summary only').replaceAll('_', ' ')}`}`;
...
export async function PredictionAccuracyPage(root) {
  ...
  async function render() {
    ...
    const experiment4 = experimentResult.status === 'fulfilled' ? experimentResult.value : null;
    const experimentStages = experimentStagesResult.status === 'fulfilled' ? experimentStagesResult.value : null;
    ...
    const runId = metadata.run_id || metadata.provenance?.run_id || selectedRun.run_id || null;
    const datasetFingerprint = metadata.dataset_fingerprint || metadata.provenance?.dataset_fingerprint || selectedRun.dataset_fingerprint || null;
    const balanced = metadata.balanced_stage_summary || report.balanced_stage_summary || {};
    ...
    root.innerHTML = `<header class="page-head"><div><span class="kicker">Historical cutoff backtest · ${escape(report.model_version || selected)}</span><h1>Prediction Accuracy Dashboard</h1><p>Metrics are loaded from the exact selected lifecycle run; later completed projects remain the out-of-time holdout.</p></div></header>
      ${selectorPanel()}
      <div class="notice"><strong>Model family:</strong> ${lifecycle ? 'Monthly Lifecycle' : 'Legacy verification'} · <strong>Training:</strong> ${metadata.training_start ?? metadata.training_period?.[0] ?? 'N/A'}-${metadata.training_end ?? metadata.training_period?.[1] ?? 'N/A'} · <strong>Future holdout:</strong> ${metadata.evaluated_test_start ?? metadata.testing_period?.[0] ?? 'N/A'}-${metadata.evaluated_test_end ?? metadata.testing_period?.[1] ?? 'N/A'}. ${metadata.leakage_policy || metadata.future_information_policy || ''}</div>
      <div class="notice compact"><strong>Run ID:</strong> ${escape(runId || 'Not recorded for this persisted artifact')} · <strong>Dataset fingerprint:</strong> ${escape(shortFingerprint(datasetFingerprint))} · <strong>Provenance:</strong> ${escape(selectedRun.provenance_status || 'unavailable')} · <strong>Generated:</strong> ${escape(metadata.created_at || selectedRun.created_at || 'Not recorded')}</div>
      <div class="stat-grid"><article class="stat-card blue"><span class="stat-eyebrow">Cost MAE</span><strong class="stat-value">${formatMetric(report.cost_model?.MAE)} pp</strong><small class="stat-note">RMSE ${formatMetric(report.cost_model?.RMSE)} · MAPE ${formatMetric(report.cost_model?.MAPE)}% · R² ${value(report.cost_model?.R2, 3)}</small></article><article class="stat-card amber"><span class="stat-eyebrow">Delay MAE</span><strong class="stat-value">${formatMetric(delayMae)} days</strong><small class="stat-note">RMSE ${formatMetric(delayRmse)} · ${lifecycle ? `MAPE ${formatMetric(report.delay_model?.MAPE)}%` : `log-target RMSE ${value(report.delay_model?.log_target_RMSE, 4)}`} · R² ${value(report.delay_model?.R2, 3)}</small></article><article class="stat-card green"><span class="stat-eyebrow">${lifecycle ? 'Risk macro-F1' : 'Risk classification'}</span><strong class="stat-value">${formatRatioPercent(lifecycle ? risk.macro_f1 : risk.accuracy)}</strong><small class="stat-note">${lifecycle ? `Accuracy ${formatRatioPercent(risk.accuracy)} · macro precision ${formatRatioPercent(risk.macro_precision)} · macro recall ${formatRatioPercent(risk.macro_recall)}` : `Precision ${formatRatioPercent(risk.precision)} · recall ${formatRatioPercent(risk.recall)} · F1 ${formatRatioPercent(risk.f1)}`}</small></article><article class="stat-card"><span class="stat-eyebrow">Model confidence</span><strong class="stat-value">${formatMetric(averageConfidence, 1, '%')}</strong><small class="stat-note">${lifecycle ? 'Unavailable unless calibrated for this run' : 'Earlier-year calibrated interval coverage'}</small></article></div>
      <div class="stat-grid"><article class="stat-card blue"><span class="stat-eyebrow">Features used</span><strong class="stat-value">${value(metadata.feature_count ?? metadata.features_used?.length, 0)}</strong><small class="stat-note">Only audited official-data features</small></article><article class="stat-card amber"><span class="stat-eyebrow">Removed invalid</span><strong class="stat-value">${value(quality.removed_invalid_feature_count, 0)}</strong><small class="stat-note">Empty, constant, synthetic, unavailable, or unsafe fields</small></article><article class="stat-card green"><span class="stat-eyebrow">Data quality</span><strong class="stat-value">${formatMetric(quality.data_quality_score, 1, '%')}</strong><small class="stat-note">Availability across retained features</small></article><article class="stat-card"><span class="stat-eyebrow">Run status</span><strong class="stat-value">${escape(String(selectedRun.status || 'unknown').replaceAll('_', ' '))}</strong><small class="stat-note">Validation rows: ${selectedRun.has_validation_rows ? 'available' : 'not generated in this checkout'}</small></article></div>
      ${lifecycle ? `<div class="notice compact"><strong>Equal-stage diagnostic:</strong> cost MAE ${formatMetric(balanced.cost_mae)} pp · delay MAE ${formatMetric(balanced.delay_mae)} days · risk macro-F1 ${formatRatioPercent(balanced.risk_macro_f1)}. This gives early/mid/late/very-late stages equal weight instead of letting very-late snapshots dominate.</div>` : ''}
      ${validationWarning}${rollingWarning}
      ${experimentStages ? `<section class="panel"><div class="panel-head"><div><span class="kicker">Experiment 4 explainability</span><h2>Specialist feature importance</h2></div><label>Lifecycle stage<select id="experiment-stage-feature" class="data-input">${['early','early_mid','late_mid','late'].map((stage) => `<option value="${stage}">${stage.replaceAll('_',' ')}</option>`).join('')}</select></label></div><div id="experiment-feature-output" class="model-grid"></div></section>` : ''}
      ${experiment4 ? `<section class="panel"><div class="panel-head"><div><span class="kicker">Experiment 4</span><h2>Lifecycle Specialist Experiment</h2></div></div><p class="muted">Separate lifecycle-stage models may outperform a single global model because predictive signals change as projects mature. One specialist is routed per snapshot; predictions are never averaged.</p><div class="stat-grid"><article class="stat-card blue"><span class="stat-eyebrow">Global cost MAE</span><strong class="stat-value">${formatMetric(experiment4.global_baseline?.metrics?.cost?.MAE)} pp</strong></article><article class="stat-card green"><span class="stat-eyebrow">Specialist cost MAE</span><strong class="stat-value">${formatMetric(experiment4.specialist_overall?.cost?.MAE)} pp</strong></article><article class="stat-card amber"><span class="stat-eyebrow">Global delay MAE</span><strong class="stat-value">${formatMetric(experiment4.global_baseline?.metrics?.delay?.MAE)} days</strong></article><article class="stat-card green"><span class="stat-eyebrow">Specialist delay MAE</span><strong class="stat-value">${formatMetric(experiment4.specialist_overall?.delay?.MAE)} days</strong></article></div><div class="table-wrap"><table class="data-table compact-table"><thead><tr><th>Stage</th><th>Range</th><th>Train projects</th><th>Test projects</th><th>Global cost MAE</th><th>Specialist cost MAE</th><th>Δ%</th><th>Global delay MAE</th><th>Specialist delay MAE</th><th>Δ%</th><th>Status</th></tr></thead><tbody>${Object.entries(experiment4.specialists || {}).map(([stage, item]) => { const row = experiment4.comparison?.[stage] || {}; return `<tr><td>${escape(stage)}</td><td>${escape((item.lifecycle_range || []).map((x) => x == null ? '75%+' : `${Number(x) * 100}%`).join('–'))}</td><td>${value(item.training_projects, 0)}</td><td>${value(item.test_projects, 0)}</td><td>${formatMetric(row.global_cost?.MAE)}</td><td>${formatMetric(row.specialist_cost?.MAE)}</td><td>${formatMetric(row.cost_improvement_pct, 1, '%')}</td><td>${formatMetric(row.global_delay?.MAE)}</td><td>${formatMetric(row.specialist_delay?.MAE)}</td><td>${formatMetric(row.delay_improvement_pct, 1, '%')}</td><td>${item.available ? 'Available' : escape(item.reason || 'Unavailable')}</td></tr>`; }).join('')}</tbody></table></div></section>` : '<section class="panel"><div class="panel-head"><div><span class="kicker">Experiment 4</span><h2>Lifecycle Specialist Experiment</h2></div></div><p class="muted">No Experiment 4 artifact is available for this window yet. Train it explicitly from the Model Simulation workflow or API.</p></section>'}
      ...
      <section class="panel"><div class="panel-head"><div><span class="kicker">Expanding-window temporal validation · ${value(rolling.fold_count, 0)} folds</span><h2>Reliability across unseen completion years</h2></div></div>${rolling.folds?.length ? `<div class="model-grid"><div><h3>Cost MAE by test year</h3>${lineChart(rolling.folds.map((fold) => ({ label: fold.test_year, value: fold.cost_MAE })), { suffix: ' pp' })}</div><div><h3>Delay MAE by test year</h3>${lineChart(rolling.folds.map((fold) => ({ label: fold.test_year, value: fold.delay_MAE_days })), { suffix: ' days' })}</div></div><div class="notice compact"><strong>Average risk macro F1:</strong> ${formatRatioPercent(rolling.average_risk_f1)}. Every fold trains only on years before its displayed test year.</div>` : '<div class="notice compact">Rolling validation not generated for this lifecycle run.</div>'}</section>
    ...
    if (experimentStages) {
      const featureSelect = root.querySelector('#experiment-stage-feature');
      const featureOutput = root.querySelector('#experiment-feature-output');
      const renderFeatureImportance = () => {
        const stage = experimentStages.stages?.[featureSelect.value] || {};
        const importance = stage.feature_importance || {};
        const list = (key) => (importance[key]?.features || []).slice(0, 8).map((item) => `<li>${escape(item.feature)} (${formatMetric(item.importance, 3)})</li>`).join('') || '<li>N/A</li>';
        featureOutput.innerHTML = `<section class="panel"><h3>${escape(featureSelect.value)} cost drivers</h3><ol>${list('cost')}</ol></section><section class="panel"><h3>${escape(featureSelect.value)} delay drivers</h3><ol>${list('delay')}</ol></section>`;
      };
      featureSelect?.addEventListener('change', renderFeatureImportance);
    ...
    const renderRows = (sortKey = 'cost_error', direction = 'asc') => {
      const sorted = [...rows].sort((a, b) => {
        ...
        const leftMissing = leftRaw === null || leftRaw === undefined || leftRaw === '' || !Number.isFinite(Number(leftRaw));
        const rightMissing = rightRaw === null || rightRaw === undefined || rightRaw === '' || !Number.isFinite(Number(rightRaw));
        if (leftMissing && rightMissing) return 0;
        if (leftMissing) return 1;
        if (rightMissing) return -1;
        const left = Math.abs(Number(leftRaw));
        const right = Math.abs(Number(rightRaw));
