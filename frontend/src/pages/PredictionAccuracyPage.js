...
export async function PredictionAccuracyPage(root) {
  ...
  const renderRows = (sortKey = 'cost_error', direction = 'asc') => {
    ...
    tableBody.innerHTML = sorted.map((row) => `<tr><td>${row.project_id || 'Not published'}</td><td>${row.project_name || 'Not reported'}</td><td>${formatMetric(row.predicted_cost_overrun, 2, '%')}</td><td>${row.predicted_cost_p10 == null || row.predicted_cost_p90 == null ? 'N/A' : `${formatMetric(row.predicted_cost_p10, 2, '%')}–${formatMetric(row.predicted_cost_p90, 2, '%')}`}</td><td>${formatMetric(row.actual_cost_overrun, 2, '%')}</td><td>${formatMetric(row.cost_error)} pp</td><td>${formatMetric(row.predicted_delay_days, 2, ' days')}</td><td>${row.predicted_delay_p10 == null || row.predicted_delay_p90 == null ? 'N/A' : `${value(row.predicted_delay_p10)}–${value(row.predicted_delay_p90)} days`}</td><td>${formatMetric(row.actual_delay_days, 2, ' days')}</td><td>${formatMetric(row.delay_error, 2, ' days')}</td></tr>`).join('');
  };
  renderRows();
