...
export async function PredictionAccuracyPage(root) {
  ...
  root.innerHTML = `<header class="page-head"><div><span class="kicker">Historical cutoff backtest · ${report.model_version || validation.model_version || 'legacy'}</span><h1>Prediction Accuracy Dashboard</h1><p>Metrics and validation rows are generated from the active selected training window, using only later completed projects for evaluation.</p></div></header>
    ...
    <div class="stat-grid"><article class="stat-card blue"><span class="stat-eyebrow">Cost MAE</span><strong class="stat-value">${value(report.cost_model.MAE)} pp</strong><small class="stat-note">RMSE ${value(report.cost_model.RMSE)} · accuracy ${value(report.cost_model.accuracy_percentage, 1)}%</small></article><article class="stat-card amber"><span class="stat-eyebrow">Delay MAE</span><strong class="stat-value">${value(delayMae)} days</strong><small class="stat-note">RMSE ${value(delayRmse)} · accuracy ${value(report.delay_model.accuracy_percentage, 1)}%</small></article><article class="stat-card green"><span class="stat-eyebrow">Risk classification</span><strong class="stat-value">${value(risk.accuracy * 100, 1)}%</strong><small class="stat-note">Precision ${value(risk.precision * 100, 1)}% · recall ${value(risk.recall * 100, 1)}% · F1 ${value(risk.f1 * 100, 1)}%</small></article><article class="stat-card"><span class="stat-eyebrow">Validation projects</span><strong class="stat-value">${validation.total}</strong><small class="stat-note">Official unseen completed projects</small></article></div>
