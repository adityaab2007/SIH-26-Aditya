...
def fit_challenger(config: ChallengerConfig, *, data: pd.DataFrame, training_start: int, training_end: int, test_end: int, production_bundle: dict, production_receipt: dict) -> dict:
    ...
    cost_features = list(contract.get("cost") or production_receipt.get("features_used") or []); delay_features = list(contract.get("delay") or production_receipt.get("features_used") or [])
    ...
    if config.strategy == "uncertainty_shrinkage":
        policy = legacy._fit_exp79(train, cost_features, algorithm); disagreement = np.vstack([m.predict(_X(test, cost_features)) for m in policy["models"]]).std(axis=0)
        experiment_cost = production_cost.copy(); high = disagreement >= policy["threshold"]; experiment_cost[high] = (1.0 - policy["alpha"]) * experiment_cost[high] + policy["alpha"] * policy["target_median"]; serializable_policy = {k: v for k, v in policy.items() if k != "models"}
    else:
    ...
    pcm = legacy._metric(test, legacy.TARGET, "production_cost"); ecm = legacy._metric(test, legacy.TARGET, "experiment_cost"); pdm = legacy._metric(test, legacy.DELAY_TARGET, "production_delay"); edm = legacy._metric(test, legacy.DELAY_TARGET, "experiment_delay")
    ci = legacy._improvement(pcm["MAE"], ecm["MAE"]); di = legacy._improvement(pdm["MAE"], edm["MAE"])
    ...
    state = {"strategy": config.strategy, "policy": policy, "serializable_policy": serializable_policy, "cost_features": cost_features, "delay_features": delay_features, "production_cost_model": production_bundle["cost"], "production_delay_model": production_bundle["delay"]}
    rid = f"{config.experiment_id}-{training_start}-{training_end}-{uuid.uuid4().hex[:10]}"
