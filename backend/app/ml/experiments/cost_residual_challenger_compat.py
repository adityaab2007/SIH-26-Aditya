...
def _require_features(frame: pd.DataFrame, features: list[str], label: str) -> None:
    ...
    if missing:
        raise ValueError(f"{label} is missing current production features: {', '.join(missing)}")
...
def fit_challenger(config: ChallengerConfig, *, data: pd.DataFrame, training_start: int, training_end: int, test_end: int, production_bundle: dict, production_receipt: dict) -> dict:
    ...
    train, raw_test = legacy.temporal_project_split(frame, int(training_start), int(training_end), int(test_end))
    ...
    if config.strategy == "uncertainty_shrinkage":
        ...
        if config.strategy == "revised_cost_reliability":
            policy = legacy._fit_exp75(oof); correction = legacy._apply_exp75(test, policy)
        elif config.strategy == "medium_project_specialist":
            policy = legacy._fit_exp76(oof); correction = legacy._apply_exp76(test, policy)
        elif config.strategy == "early_financial_surrogate":
            policy = legacy._fit_exp77(oof); correction = legacy._apply_exp77(test, policy)
        elif config.strategy == "cross_window_consensus":
            policy = legacy._fit_exp78(oof); correction = legacy._apply_exp78(test, policy, production_cost)
        else:
    ...
    test["production_cost"] = production_cost; test["experiment_cost"] = experiment_cost
    test["production_delay"] = production_delay; test["experiment_delay"] = experiment_delay
...
def predict_row(row: pd.DataFrame, runtime_state: dict) -> dict:
    ...
    if strategy == "revised_cost_reliability": candidate_cost = prod_cost + legacy._apply_exp75(row, policy)
        ...
        disagreement = np.vstack([m.predict(_X(row, cost_features)) for m in policy["models"]]).std(axis=0)
        candidate_cost = prod_cost.copy(); high = disagreement >= policy["threshold"]
