...
Only training influence changes. Production feature/target contracts,
...
def compute_project_recency_weights(
    frame: pd.DataFrame,
    training_end_year: int,
    half_life: int | None,
) -> pd.Series:
    """Return one normalized training-side weight per project."""
...
def apply_project_recency_weights(
    frame: pd.DataFrame,
    training_end_year: int,
    half_life: int | None,
) -> tuple[pd.DataFrame, pd.Series]:
    """Distribute each project's normalized weight across retained snapshots."""
    result = frame.copy()
    ...
    result["sample_weight"] = (
        keys.map(project_weights).to_numpy(dtype=float)
        / keys.map(counts).to_numpy(dtype=float)
    )
    ...
    expected = project_weights.loc[observed.index]
    if not np.allclose(observed.to_numpy(), expected.to_numpy(), rtol=0, atol=1e-10):
...
def _candidate_validation(
    ...
    metric_key = "cost_mae" if target == "actual_cost_overrun_percentage" else "delay_mae"
    ...
    winner = min(
        ...
        key=lambda row: (
            row[metric_key],
            row["RMSE"],
            CANDIDATE_HALF_LIVES.index(row["half_life"]),
        ),
    )
...
def _weight_diagnostics(
    ...
    _, projects = apply_project_recency_weights(frame, training_end, half_life)
    values = projects.to_numpy(dtype=float)
    grouped = (
        frame.assign(_weight=frame["canonical_project_id"].astype(str).map(projects))
        .groupby("completion_year")["_weight"]
        .first()
    )
    denominator = np.square(values).sum()
    effective = float(values.sum() ** 2 / denominator) if denominator else None
    examples = {
        str(int(training_end - age)): round(float(0.5 ** (age / half_life)), 6) if half_life else 1.0
        for age in (0, 5, 10, 15)
        if training_end - age >= int(frame.completion_year.min())
    }
    return {
        "selected_half_life": half_life,
        "minimum_project_weight": round(float(values.min()), 6),
        "maximum_project_weight": round(float(values.max()), 6),
        "median_project_weight": round(float(np.median(values)), 6),
        "mean_project_weight": round(float(values.mean()), 6),
        "effective_weighted_project_sample_size": round(effective, 4) if effective is not None else None,
        "total_projects": int(len(projects)),
        "project_weight_distribution_by_completion_year": {
            str(int(k)): round(float(v), 6) for k, v in grouped.items()
        },
        "formula_examples": examples,
        "snapshot_weight_sum_policy": "one normalized project weight divided by retained training snapshot count",
    }
...
def _window_verdict(cost_improvement: float | None, delay_improvement: float | None) -> str:
    ...
    if cost_improvement > 0 and delay_improvement > 0:
...
def fit_against_production(
    *,
    ...
    contract = {
        key: list(value or production_receipt.get("features_used") or [])
    ...
    algorithms = {
        target: _algorithm(production_bundle, production_receipt, target)
    ...
    for target, target_column in (
        ...
        selected_half_lives[target], candidate_validation[target] = _candidate_validation(
            base_train,
            contract[target],
            target_column,
            algorithms[target],
    ...
    test["production_delay"] = np.maximum(
        0,
        production_bundle["delay"].predict(test[contract["delay"]]),
    )
    test["experiment_delay"] = np.maximum(
        0,
        models["delay"].predict(test[contract["delay"]]),
    )

    production_cost = _metric(test, "actual_cost_overrun_percentage", "production_cost")
    experiment_cost = _metric(test, "actual_cost_overrun_percentage", "experiment_cost")
    production_delay = _metric(test, "actual_delay_days", "production_delay")
    experiment_delay = _metric(test, "actual_delay_days", "experiment_delay")
    ...
    cost_improvement = (
        (production_cost["MAE"] - experiment_cost["MAE"]) / production_cost["MAE"] * 100
        if production_cost["MAE"] else None
    )
    delay_improvement = (
        (production_delay["MAE"] - experiment_delay["MAE"]) / production_delay["MAE"] * 100
        if production_delay["MAE"] else None
    )

    overall = {
        "production_cost_mae": production_cost["MAE"],
        "experiment_cost_mae": experiment_cost["MAE"],
        "absolute_cost_improvement_pp": round(production_cost["MAE"] - experiment_cost["MAE"], 4),
        "cost_improvement_percentage": round(cost_improvement, 4) if cost_improvement is not None else None,
        "production_delay_mae": production_delay["MAE"],
        "experiment_delay_mae": experiment_delay["MAE"],
        "absolute_delay_improvement_days": round(production_delay["MAE"] - experiment_delay["MAE"], 4),
        "delay_improvement_percentage": round(delay_improvement, 4) if delay_improvement is not None else None,
        ...
        "selected_cost_half_life": selected_half_lives["cost"],
        "selected_delay_half_life": selected_half_lives["delay"],
        "candidate_internal_validation": candidate_validation,
        "age_weight_diagnostics": {
            "cost": _weight_diagnostics(base_train, int(training_end), selected_half_lives["cost"]),
            "delay": _weight_diagnostics(base_train, int(training_end), selected_half_lives["delay"]),
        },
        "verdict": _window_verdict(cost_improvement, delay_improvement),
    }
    ...
    context = build_experiment_context(
        ...
        weighting_policy="project-balanced snapshots with normalized project-level exponential recency influence",
    )
    manifest = new_experiment_manifest(
        context=context,
        name=EXPERIMENT_NAME,
        changed_dimension="weighting",
        hypothesis="More recent completed projects may better represent future cost and delay relationships while retaining older projects.",
    )
    manifest.update({
        "scope": EXPERIMENT_SCOPE,
        "production_run_id": production_receipt.get("run_id"),
        "selected_algorithms": algorithms,
        "candidate_half_lives": list(CANDIDATE_HALF_LIVES),
        "selected_half_lives": selected_half_lives,
        "candidate_internal_validation": candidate_validation,
        "random_seeds": RANDOM_SEEDS,
        "controlled_variables": [
            "raw PAIMANA data",
            "processed lifecycle dataset",
            "project identities",
            "features",
            "targets",
            "algorithms",
        ...
        "leakage_policy": "Weights use only each training project's completion year and the training cutoff; final outcomes and future holdout rows never influence weights or half-life selection.",
        "evaluation_weighting_policy": "Production and experiment use identical project-balanced test weights.",
        "weighting_formula": "0.5 ** ((training_end_year - project_completion_year) / half_life), normalized to mean one, then divided by retained snapshots per project.",
    })
    ...
    experiment = {
        ...
        "candidate_half_lives": list(CANDIDATE_HALF_LIVES),
        "age_weight_diagnostics": overall["age_weight_diagnostics"],
        "verdict": overall["verdict"],
...
def _key(row: pd.Series):
    ...
    if pd.isna(project) or pd.isna(date):
        return None
...
def predict_project(row: pd.Series, state: dict) -> dict:
    ...
    return {
        ...
        "selected_cost_half_life": state.get("selected_cost_half_life"),
        "selected_delay_half_life": state.get("selected_delay_half_life"),
    }
