...
EXP_ID = "exp126"
_AFT_CAPACITY_ERROR = re.compile(
    r"^Only (\d+) projects have AFT evidence; cannot form the requested (\d+)-project calibration cohort\.$"
)


def _run_with_adaptive_forward_oof(output_dir) -> None:
    """Adapt only Exp126's rolling-OOF audit gate to the fold's AFT capacity."""
            ...
            try:
                ...
                if available < 20:
                    raise RuntimeError(
                        ...
                        "Exp126 requires at least 20 for an experiment-local adaptive calibration gate."
                    ) from exc
    ...
    if reductions:
        print(
            "EXP126_OOF_AFT_GATE_ADAPTATIONS="
...
def main() -> None:
    ...
    p.add_argument("--output-dir", default="models/monthly_lifecycle/experiments/exp126/ci")
